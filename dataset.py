...
class UCSD_Ped(object):
    def __init__(self, flags, dataset_name):
        self.flags = flags
        self.dataset_name = dataset_name
        self.image_size = (158, 238, 3)
        self.input_height = self.input_width = 164
        self.num_trains = 0

        self.ucsd_path = os.path.join('../Data', self.dataset_name)
        self._load_ucsd_ped()

    def _load_ucsd_ped(self):
        print('Load {} dataset...'.format(self.dataset_name))

        self.train_data = utils.getListOf_imgFiles(self.ucsd_path)
        self.num_trains = len(self.train_data)
        print('Load {} dataset SUCCESS!'.format(self.dataset_name))
...
def Dataset(sess, flags, dataset_name):
    ...
    elif dataset_name == 'ucsd_ped/test':
        return UCSD_Ped(flags, dataset_name)
    else:
