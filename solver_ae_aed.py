...
class Solver(object):
    ...
    def test(self):
        ...
        for iter_time in range(num_iters):
            ...
            imgs, input, gt = self.model.sample_imgs()  # inference
            total_time += time.time() - start_time
            #self.model.plots(imgs, iter_time, self.test_out_dir)
            self.model.epn_test_plots(imgs,input,gt, iter_time, self.test_out_dir)
    ...
    def sample(self, iter_time):
        if np.mod(iter_time, self.flags.sample_freq) == 0:
            imgs, input, gt  = self.model.sample_imgs()
            #self.model.plots(imgs, iter_time, self.sample_out_dir)
            self.model.epn_plots(imgs,input,gt, iter_time, self.sample_out_dir)
    ...
    def load_model_force(self):
        print(' [*] Reading checkpoint...')
        self.saver.restore(self.sess, '/home/neumann/Workspace/aed_wgan/WGAN-TensorFlow/src/ucsd_ped/UCSDped1/Train/model/20190827-1444/model-36999')
        return True