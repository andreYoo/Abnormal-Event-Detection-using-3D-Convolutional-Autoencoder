...
def linear(x, output_size, bias_start=0.0, with_w=False,input_size=None, name='fc'):
    if input_size!=None:
        with tf.variable_scope(name):
            matrix = tf.get_variable(name="matrix", shape=[input_size, output_size],
...
def upsampling3d(x, size=(2, 2), name='upsampling3d'):
    with tf.name_scope(name):
        shape = x.get_shape().as_list()
        return tf.image.resize_nearest_neighbor(x, size=(size[0] * shape[1], size[1] * shape[2]))

def conv3d(x, output_dim,k_d=2, k_h=4, k_w=4,d_t=2, d_h=2, d_w=2, stddev=0.02, padding='SAME', name='conv3d', is_print=True):
    with tf.variable_scope(name):
        w = tf.get_variable('w', [k_d, k_h, k_w, x.get_shape()[-1], output_dim], initializer=tf.truncated_normal_initializer(stddev=stddev))
        conv = tf.nn.conv3d(x, w, strides=[1,d_t, d_h, d_w, 1], padding=padding)
        biases = tf.get_variable('biases', [output_dim], initializer=tf.constant_initializer(0.0))
        # conv = tf.reshape(tf.nn.bias_add(conv, biases), conv.get_shape())
        conv = tf.nn.bias_add(conv, biases)
        if is_print:
...
def deconv3d(x, k, k_t=2, k_h=4, k_w=4,d_t=1, d_h=2, d_w=2,stepup_out = 1, stddev=0.02, padding_='SAME', output_size=None,
             name='deconv3d', with_w=False, is_print=True):
    with tf.variable_scope(name):
        input_shape = x.get_shape().as_list()
        ...
        h_output, w_output = None, None
        if not output_size:
            t_output, h_output, w_output = input_shape[1]*stepup_out, input_shape[2] * 2, input_shape[3] * 2
        # output_shape = [input_shape[0], h_output, w_output, k]  # error when not define batch_size
            output_shape = [tf.shape(x)[0],t_output, h_output, w_output, k]
        else:
            output_shape = [tf.shape(x)[0], 5, output_size[0], output_size[1], output_size[2]]

        # conv2d transpose
        w = tf.get_variable('w', [k_t, k_h, k_w, k, input_shape[4]], initializer=tf.random_normal_initializer(stddev=stddev))
        deconv = tf.nn.conv3d_transpose(x, w, output_shape=output_shape, strides=[1,d_t, d_h, d_w, 1],padding=padding_)
        biases = tf.get_variable('biases', [output_shape[-1]], initializer=tf.constant_initializer(0.0))
        deconv = tf.nn.bias_add(deconv, biases)
        if is_print:
            print_activations(deconv)
